    ...
    <div class="col-span-12 mt-4">
        <div class="" style="display: flex;justify-content: space-between">
            ...
            <div class="" style="display: flex; justify-content: flex-end;">
                <div>
                    <form action='{{ route('create.report.form.complainfilter') }}' method="post" enctype="multipart/form-data">
                        @csrf
                        <table>
                            <tbody>
                                <tr>
                    ...
                    <div class="box p-5">
                        ...
                        if (session()->get('fromdate') != '' && session()->get('todate')) {
                            $complaintscount = DB::table('complaints')
                                ...
                                ->whereBetween('complaints.created_at', [session()->get('fromdate') . " 00:00:00", session()->get('todate') . " 23:59:59"])
                                ->count();
                        } else {
                            $complaintscount = DB::table('complaints')
                                ->join('users', 'complaints._customer_id', '=', 'users.id')
                        ...
                        <div class="mt-6 text-3xl font-medium leading-8">{{ count($jobs) }}</div>
                            ...
                            <div class="ml-auto">
                                ...
                                @php
                                    if (session()->get('fromdate') != '' && session()->get('todate')) {
                                        ...
                                        //     ->select('complaints.*', 'users.name', 'customer_details.location_type')
                                        //     ->get();
                                        $complaintscount = DB::table('jobs')->join('users', 'jobs.customer_id', '=', 'users.id')
                                            ->join('complaints', 'jobs.complaint_id', '=', 'complaints.id')
                                            ->where('complaints.amc_type','=','No AMC')
                        ...
                        @php
                            if (session()->get('fromdate') != '' && session()->get('todate')) {
                                $complaintscount = DB::table('jobs')
                                    ...
                                    ->whereBetween('created_at', [session()->get('fromdate') . " 00:00:00", session()->get('todate') . " 23:59:59"])
                                    ->count();
                            } else {
                                $complaintscount = DB::table('jobs')
                                    ->where('status', 'Complete')
                ...
                <x-base.table.tbody>
                    ...
                    @foreach ($jobs as $row)
                        ...
                        <x-base.table.tr class="intro-x text-center">
                            ...
                            <x-base.table.td
                                class="w-10 border-b-0 bg-white shadow-[20px_3px_20px_#0000000b] first:rounded-l-md last:rounded-r-md dark:bg-darkmode-600">
                                <a href="{{ route('complainreport.list',$row->customer_id) }}">
                                    {{ $row->name }}
                                </a>
                            </x-base.table.td>
