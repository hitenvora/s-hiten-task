...
<div class="grid grid-cols-12 gap-6 mt-5" >
        <div class="intro-y col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
            ...
            <h2 class="text-lg font-medium intro-y">List Job</h2>
        ...
        <div class="col-span-12 overflow-auto intro-y 2xl:overflow-visible">
            ...
            <x-base.table class="-mt-2 border-separate border-spacing-y-[10px] content-table" style="margin-top: 0px;">
                ...
                <x-base.table.tbody >
                    ...
                    if(Request::get('search') != '' && $status != '')
                    {
                        $value = Request::get('search');

                        $jobs = DB::table('jobs')->join('users', 'jobs.customer_id', '=', 'users.id')->select('jobs.*', 'users.name')->Where('users.name', 'like', "%{$value}%")->orWhere('jobs.job_ref_no', 'like', "%{$value}%")->where('jobs.status','<>','Complete')->orderBy('id', 'DESC')->get();
                    }
                    else if($status != '')
                    {
                        if($status == 'all')
                        {
                            ...
                            ->join('customer_details', 'customer_details.id', '=', 'users.id')
                            ->select('jobs.*', 'users.name', 'customer_details.address', 'customer_details.id as customer_detail_id', 'customer_details.location_type')
                        ...
                        <x-base.table.tr class="intro-x text-center">
                            ...
                            <x-base.table.td  class="w-10 border-b-0 bg-white shadow-[20px_3px_20px_#0000000b] first:rounded-l-md last:rounded-r-md dark:bg-darkmode-600" >
                                ...
                                @if($row->status == 'Assign')
                                    <div class="flex items-center justify-center" style="color: #4087d3!important;">
                            ...
                            <x-base.table.td  class="w-10 border-b-0 bg-white shadow-[20px_3px_20px_#0000000b] first:rounded-l-md last:rounded-r-md dark:bg-darkmode-600" >
                                ...
                                <a class="flex items-center mb-2 text-success"
                            ...
                            <form action="{{ route('job.destroy', $row->id) }}" method="POST" onsubmit="return confirm('Delete Job')">
                                @csrf
                                <button type="submit" class="px-2 py-1 flex items-center text-danger">
...
<div style="display:none;">
        <div class="col-span-12 overflow-auto intro-y 2xl:overflow-visible" id="invoice">
            ...
            <x-base.table class="-mt-2 border-separate border-spacing-y-[10px] content-table" id="default-datatable" style="margin-top: 0px;">

                <x-base.table.thead class='text-center' style='background-color: #164E63; color: #ffffff; text-align: left; font-weight: bold;'>

                    <x-base.table.tr>

                        <x-base.table.th class="border-b-0 whitespace-nowrap text-center" style="padding:10px;">
