...
<div class="grid grid-cols-12 gap-6 mt-5">
        ...
        <h2 class="mt-10 text-lg font-medium intro-y">Up Coming AMC</h2>
         <div class="mx-auto hidden text-slate-500 md:block">
               
        </div>
        <div class="mt-3 w-full sm:mt-0 sm:ml-auto sm:w-auto md:ml-0" style="display: flex;">
                <div class="relative w-56 text-slate-500">
                    <form method="get" action="/search-upcomming">
                        <x-base.form-input
                ...
                <x-base.table.tbody >
                                    ...
                                    if(Request::get('search') != '')
                    {
                        ...
                        $amcs = DB::table('amcs')
                                    ...
                                    ->whereBetween('amc_visit.visit_date', [$currentDate, $thirtyDaysLater])->Where('users.name', 'like', "%{$value}%")->orWhere('amcs.amc_type', 'like', "%{$value}%")->orWhere('amcs.contract_amount', 'like', "%{$value}%")->orderBy('amcs.id', 'DESC')->simplePaginate(20);
                    }
                    else
                    {
                        $amcs = DB::table('amcs')
                    ...
                    @endphp
                        ...
                        <x-base.table.tr class="intro-x">
                            ...
                            <x-base.table.td  class="w-10 border-b-0 bg-white shadow-[20px_3px_20px_#0000000b] first:rounded-l-md last:rounded-r-md dark:bg-darkmode-600" >
                                <!-- <a class='btn btn-success' href='{{ route('create.complaint',['id'=>$row->_user_id]) }}'>Create Complain</a> -->
                                <a
                                                    class=" flex"
                                                    href="{{ route('create.complaint',['amcvisit_id'=>$row->id]) }}"
                                                >
                                                    <x-base.lucide
                                                        class="mr-1 h-4 w-4"
                                                        icon="CheckSquare"
                ...
                <x-base.table.tbody >
                                    ...
                                    if(Request::get('search') != '')
                    {
                        ...
                        $amcs = DB::table('amcs')
                                    ...
                                    ->join('amc_visit', 'amc_visit._amc_id', '=', 'amcs.id')
                                    ->select('users.name', 'amcs.*', 'amc_visit.*')
                                    ->whereBetween('amc_visit.visit_date', [$currentDate, $thirtyDaysLater])->Where('users.name', 'like', "%{$value}%")->orWhere('amcs.amc_type', 'like', "%{$value}%")->orWhere('amcs.contract_amount', 'like', "%{$value}%")->simplePaginate(20);
                    }
                    else
                    {
                        $amcs = DB::table('amcs')
