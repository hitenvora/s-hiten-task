            ...
            <div class="intro-y col-span-12 md:col-span-6">
                <div class="box">
                    <div class="flex flex-col items-center p-5 lg:flex-row">
                        ...
                        <form id="deleteForm_{{$data->id}}" action="{{ route('del.product', ["id" => $data->id]) }}" method="GET">
                            @csrf
                            <input type="hidden" name="id" value="{{ $data->id }}" />
                            <button data-tw-merge type="button" onclick="confirmDelete({{$data->id}})" class="transition duration-200 border shadow-sm inline-flex items-center justify-center py-2 px-3 rounded-md font-medium cursor-pointer focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus-visible:outline-none dark:focus:ring-slate-700 dark:focus:ring-opacity-50 [&amp;:hover:not(:disabled)]:bg-opacity-90 [&amp;:hover:not(:disabled)]:border-opacity-90 [&amp;:not(button)]:text-center disabled:opacity-70 disabled:cursor-not-allowed bg-danger border-danger text-white dark:border-danger mb-2 mr-1">
                                <i data-lucide="trash" width="24" height="24" class="stroke-1.5 h-5 w-5"></i>Delete
                            </button>
                        </form>
                        </div>
                    </div>
