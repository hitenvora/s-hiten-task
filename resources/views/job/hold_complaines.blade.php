...
@section('subcontent')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
            ...
            <h2 class="text-lg font-medium intro-y">List Hold Compliance</h2>
        ...
        <div class="col-span-12 overflow-auto intro-y 2xl:overflow-visible">
            <x-base.table class="-mt-2 border-separate border-spacing-y-[10px] content-table" style="margin-top: 0px;">
                ...
                <x-base.table.tbody>
                    ...
                    @php $i = 0; @endphp
                    @foreach ($hold_complaints as $row)
                        @php
                            $i++;
                        @endphp
                        <x-base.table.tr class="intro-x text-center">
                            ...
                            {{-- <x-base.table.td
                                ...
                                @if ($row->address != '0')
                                    @php
                                        $address = DB::table('customer_details')
                            ...
                            {{-- <x-base.table.td
                                class="w-10 border-b-0 bg-white shadow-[20px_3px_20px_#0000000b] first:rounded-l-md last:rounded-r-md dark:bg-darkmode-600">
                                <form id="deleteForm-{{ $row->id }}" action="{{ route('job.destroy', $row->id) }}"
                                    method="POST">
                                    @csrf
                                    @method('DELETE') <!-- Method Spoofing -->
                                    <button type="button" onclick="deleteJob('{{ $row->id }}')"
                                        class="px-2 py-1 flex items-center text-danger">
